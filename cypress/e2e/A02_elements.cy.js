describe('Central de Atendimento ao Cliente TAT', () => {
    ...
    it('Preencher e limpar os campos digitáveis.', () => {
      cy.get('#firstName').type('Pafuncio').should('have.value', 'Pafuncio').clear().should('have.value', '')
      cy.get('input[name="lastName"]').type('Fulanovich').should('have.value', 'Fulanovich').clear().should('have.value', '')
      cy.get('input[type="email"]').type('pafuncio@terra.com').should('have.value', 'pafuncio@terra.com').clear().should('have.value', '')
      cy.get('textarea').type('This is a feedback.', { delay: 0}).should('have.value', 'This is a feedback.').clear().should('have.value', '')
      cy.get('input[name="phone"][type="number"]').type('1140028922').should('have.value', '1140028922').clear().should('have.value', '')
    })
